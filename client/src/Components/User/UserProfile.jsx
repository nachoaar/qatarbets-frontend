...
export const UserProfile = ({ modal, setModal }) => {
  ...
  function handleModifyButton(e) {
    e.preventDefault(setModify(!modify));
  }
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col justify-between w-full sm:w-1/2 h-full bg-morado relative rounded-lg overflow-hidden shadow p-4 sm:p-5 text-white">
          <div>
            <div className="flex items-center content-between mb-3 pb-3 border-b border-rojo">
              <TitleContent title={"Mi perfil"} />
            </div>
            <button
              onClick={(e) => handleOnClick(e)}
              className="absolute transition duration-200 hover:bg-gris rounded top-4 right-5 w-6 h-6 flex items-center justify-center"
            >
              <svg
                className="fill-rojo"
          ...
          <div className="w-full flex flex-wrap justify-between items-end pb-2 sm:p-0">
            <div className="w-full h-auto mb-2 flex flex-wrap justify-start items-center gap-4">
              <div className="w-32 h-32 overflow-hidden border-4 border-amarillo rounded-xl">
                <img
                  className="h-32 w-32"
              ...
              <div className="flex flex-col gap-2">
                {modify === true ? (
                  ...
                  <p className="border-b border-rojosec w-40 font-titulodash">
                    {user.length >= 1 ? user[0]?.name.toUpperCase() : "Nombre"}
                  </p>
                )}
                <p className="border-b border-rojosec w-40">
                  {user.length >= 1 ? `${user[0].age} años` : "18+"}
                </p>
                <p className="border-b border-rojosec">
                  {user.length >= 1 ? user[0].email : "example@example.com"}
                </p>
              </div>
            </div>
            {modify === true ? (
          ...
          <div className="w-full h-3/5 overflow-auto">
            <div className="w-full h-auto p-2 bg-gray-100 overflow-auto border-t border-rojo flex flex-col gap-2">
              <p className="w-full border-b border-rojo pb-2 font-titulodash font-bold text-rojosec">
                Mis apuestas
              </p>
              {bets &&
                bets?.map((bet) => {
              ...
              <p className="w-full border-y border-rojo py-2 font-titulodash font-bold text-rojosec">
                Mi estadística
              </p>
              <UserStatGafic
